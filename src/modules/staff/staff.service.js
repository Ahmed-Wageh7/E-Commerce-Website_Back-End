...
const createStaff = async (payload) => {
  const user = await User.findOne({ _id: payload.user, isDeleted: false });
  ...
  return { message: "Staff member created", staff };
};
...
const getStaffDetails = async (id) => {
  const staff = await Staff.findOne({ _id: id, isDeleted: false }).populate("user");
  if (!staff) throw new AppError("Staff member not found", 404);
  return { staff };
};
...
const deleteStaff = async (id) => {
  ...
  return { message: "Staff soft deleted" };
};

const calculateSalary = async (id, month) => {
  const staff = await getStaffById(id);
  ...
  return buildSalarySummary(staff, attendances, deductions, month);
};

const markSalaryPaid = async (id, month) => {
  const staff = await getStaffById(id);
  ...
  const deductions = await Deduction.find({ staff: staff._id, month });
  const salarySummary = buildSalarySummary(staff, summary, deductions, month);
  ...
  await staff.save();
  return { message: "Salary marked as paid", report: payload };
};

const adjustSalary = async (id, month, finalSalary) => {
  const staff = await getStaffById(id);

  const reportIndex = staff.monthlyReports.findIndex((entry) => entry.month === month);
  const previousAdjustment =
    reportIndex === -1 ? 0 : staff.monthlyReports[reportIndex].adjustmentAmount || 0;
  const updatedAdjustment = previousAdjustment + finalSalary;
  const attendances = await Attendance.find({
    staff: staff._id,
    date: { $regex: `^${month}` }
  });
  ...
  if (reportIndex === -1) {
    staff.monthlyReports[staff.monthlyReports.length - 1].totalDaysWorked =
      salarySummary.totalDaysWorked;
    staff.monthlyReports[staff.monthlyReports.length - 1].totalDeductions =
      salarySummary.totalDeductions;
    staff.monthlyReports[staff.monthlyReports.length - 1].finalSalary =
  ...
  return {
    ...
    report: staff.monthlyReports[
      reportIndex === -1 ? staff.monthlyReports.length - 1 : reportIndex
    ]
  };
};
